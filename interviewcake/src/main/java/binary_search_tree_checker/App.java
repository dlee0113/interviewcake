package binary_search_tree_checker;
...
//		b.display(b.root);
//		System.out.println();
		System.out.println(isValidBST(b.root));
		
		BinarySearchTree invalidBST = new BinarySearchTree();
		invalidBST.root = new Node(3);
		invalidBST.root.left = new Node(5);
		invalidBST.root.right = new Node(1);
		
		System.out.println("Original Tree : ");
		b.display(b.root);
		System.out.println();
		System.out.println(isValidBST(invalidBST.root));
	}
	...
	public static boolean isValidBST(Node root, int max, int min) {
		...
		if (root.data <= min || root.data >= max)
			return false;
		return isValidBST(root.left, Math.min(max, root.data), min)
				&& isValidBST(root.right, max, Math.max(min, root.data));
	}
}
